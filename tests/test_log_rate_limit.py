...
class TestLogRateLimit(unittest.TestCase):
    ...
    def test_log_limit_default_unaffected(self) -> None:
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(7)])
        # Confirm there are no duplicated log lines at all.
        log_lines = self._caplog.text.splitlines()
        assert len(log_lines) == len(set(log_lines))

    def test_log_limit_filter_line_no(self) -> None:
        ...
        assert all([line in self._caplog.text for line in generate_lines(3)])
        log_lines = self._caplog.text.splitlines()
        # Confirm there are no duplicated log lines at all.
        assert len(log_lines) == len(set(log_lines))

    def test_log_limit_filter_log_message(self) -> None:
        ...
        log_full = self._capsys.readouterr().err
    ...
    def test_log_limit_filter_undefined(self) -> None:
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(2)])

    @patch("log_rate_limit.log_rate_limit.TEST_MODE", True)
    def test_log_limit_streams(self) -> None:
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(4)])

    @patch("log_rate_limit.log_rate_limit.TEST_MODE", True)
    def test_log_limit_dynamic_period_sec(self):
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(6)])

    @patch("log_rate_limit.log_rate_limit.TEST_MODE", True)
    def test_log_limit_summary(self):
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(3)])
        assert "\n + skipped 3 logs due to rate-limiting" in self._caplog.text
        assert "skipped 2 logs" not in self._caplog.text
        assert "Some logs missed" in self._caplog.text

    @patch("log_rate_limit.log_rate_limit.TEST_MODE", True)
    def test_log_limit_allow_next_n(self):
        """Test the summary functionality."""
        # Setup logging.
        _log = logging.getLogger(get_test_name())
        _log.setLevel(logging.INFO)
        ...
        # Setup to filter all logs in the same stream without needing to define a stream_id each time.
        _log.addFilter(
            StreamRateLimitFilter(1, default_stream_id=DefaultSID.NONE, filter_undefined=True, allow_next_n=2)
        )
        ...
        assert "___" not in self._caplog.text
        assert all([line in self._caplog.text for line in generate_lines(5)])
    ...
    def test_expiry(self):
        ...
        assert "[Previous logs] 1 logs were skipped" in self._caplog.text

    def test_expiry_on_skipped_message(self):
    ...
    def test_clear_with_custom_time(self):
    ...
    def test_limited_stream_id_length(self):
